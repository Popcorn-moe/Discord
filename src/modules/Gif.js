...
export default class Gif
{
	...
	setup()
	{
		Object.entries(settings)
			.forEach(([name, cmd]) => this.setupOne(name, cmd));
	}
	...
	setupOne(name, { desc, msg, gifs })
	{
		...
		const value = (message, mention, name, id) =>
		{
			...
			if (mention)
				...
				this.response(message, msg, gifs, member, members.byName(guild, name, id));
			else
				this.response(message, msg, gifs, guild.me, member);
		};
	...
	response(message, msg, gifs, from, to)
	{
		...
		const send = msg.replace('{0}', from.displayName).replace('{1}', to.displayName);
